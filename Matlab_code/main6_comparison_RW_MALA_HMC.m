...
%X  = sample_Langevin(numberOfSteps, dt, dV , X0, zeros(size(X0)));
[X, rejections] = sample_HMC(numberOfSteps, dt, T, V, dV , X0);
...
%ylim([0,1])
%xlim([0,max(lag)])
title(['T = ', num2str(T),', dt = ', num2str(dt),', ',num2str(rejections), ' rejections, rate ', num2str(rejections / length(X))])
set(gca, 'FontSize', myFontSize)
